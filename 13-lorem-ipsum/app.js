...
form.addEventListener('submit', (e) => {
  ...
  if (!amount.value || value < 0 || value > 9) {
    alert('Please enter a number between 0-9')
  } else {
    if (value === 1) {
      const random = Math.floor(Math.random() * text.length)
      resultText.innerHTML = `<p>${text[random]}</p>`
    } else {
      const textList = text.slice(0, amount.value)
      const pharagraphs = textList
        .map((item) => {
          return `<p class="result">${item}</p>`
        })
        .join('')

      resultText.innerHTML = pharagraphs
    }
  }
})
